...
public class CuboidRegion {
    ...
    public void extendBiome(Biome biome, int blockPerTick, Runnable runnable) {
        forEachBlockLimited(blockPerTick, block -> {
            block.setBiome(biome);
        }, runnable);
    }


    public void forEachHighestBlock(Consumer<Block> blockConsumer) {
        for (int i = bot.getBlockX(); i < top.getBlockX(); i++) {
            for (int j = bot.getBlockZ(); j < top.getBlockZ(); j++) {
                Block highestBlockAt = bot.getWorld().getHighestBlockAt(i , j );
                if (!highestBlockAt.getType().isAir()) {
                    blockConsumer.accept(highestBlockAt);
                }
            }
        }
    }

    public Block getRandomSafeLand(int maxTry) {
        if (maxTry <= 0) {
            return null;
        }
        int x = bot.getBlockX() + new Random().nextInt(top.getBlockX() - bot.getBlockX());
        int z = bot.getBlockZ() + new Random().nextInt(top.getBlockZ() - bot.getBlockZ());
        Block highestBlockAt = bot.getWorld().getHighestBlockAt(x, z);
        if (highestBlockAt.getType().isAir()) {
            return getRandomSafeLand(maxTry - 1);
        }
        if (!highestBlockAt.getType().isSolid()) {
            return getRandomSafeLand(maxTry - 1);
        }
        if (highestBlockAt.getType().isBurnable()) {
            return getRandomSafeLand(maxTry - 1);
        }
